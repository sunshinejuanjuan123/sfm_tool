...
def select_seeds(dist1: torch.Tensor, R1: float, scores1: torch.Tensor, fnn12: torch.Tensor, mnn: torch.Tensor):
    ...
    """
    # 原本adalam的思想是找到半径r内，score（比率校验值）值最小的匹配作为种子点, 且该匹配的score必须小于某个阈值（0.8**2)。
    # superglue中的score值是越大则匹配越好，与adalam有冲突，所以修改选择score值小改为选择score值大的匹配。
    # 而且暂时不知道superglue的score值该多大才表示该匹配好，所以暂时不设置阈值。

    im1neighmap = dist1 < R1**2  # (n1, n1)
    # find out who scores higher than whom
    # 这里原来是大于号，为了适应superglue，改为小于号。
    ...
    # 因为暂时不知道superglue的score值该多大才表示该匹配好，所以暂时不设置阈值。
    if mnn is not None:
        ...
        #                                                           0.8**2)
        im1bs = ~torch.any(im1neighmap & im1scorescomp, dim=1)
